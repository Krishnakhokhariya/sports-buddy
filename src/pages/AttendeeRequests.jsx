...
function AttendeeRequests() {
  ...
  useEffect(() => {
    async function loadEvent() {
      try {
        ...
        if (!eventSnap.exists()) {
          await showPopup("Event not found!");
          navigate("/dashboard");
          return;
        }
        ...
        if (eventData.createdBy !== profile?.uid && profile?.role !== "admin") {
          await showPopup("You don't have permission to view this page!");
        ...
        await showPopup("Error loading event");
        navigate("/dashboard");
      } finally {
        setLoading(false);
      }
  ...
  }, [eventId, profile, navigate, showPopup]);
  ...
  async function handleAccept(attendeeId) {
    const confirmed = await showConfirm("Accept this attendee request?");
    if (!confirmed) return;
    ...
    try {
      ...
      await showPopup("Request accepted successfully!");
    } catch (err) {
      console.error("Error accepting request:", err);
      await showPopup("Error accepting request");
    } finally {
      setProcessing(null);
    }
  }
  ...
  async function handleReject(attendeeId) {
    ...
    try {
      ...
      await showPopup("Request updated successfully.");
    } catch (err) {
      console.error("Error rejecting request:", err);
      await showPopup("Error rejecting request");
    } finally {
      setProcessing(null);
    }
  }
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 text-sm">
                            ...
                            <div className="flex items-start">
                              ...
                              <span className="text-gray-600 flex items-center gap-2">
                                ...
                                {isCityMatch && (
                                  <span className="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs text-center font-medium">
                                    Matches City
                                  </span>
                                )}
                              </span>
                        ...
                        {attendee.status === "accepted" && (
                          <div className="flex flex-col sm:flex-row md:flex-col gap-2 md:w-32 w-full">
                            <div
                              className="flex items-center justify-center h-10 w-full
      bg-green-50 text-green-800 rounded-lg font-semibold text-sm"
                            >
                              Accepted
                            </div>
