...
import { leaveEvent, deleteEvent, joinEvent, getEventAttendeeRequests } from "../utils/events";
...
function EventDetail() {
  ...
  const Row = ({ label, value }) => (
    <div className="flex flex-col sm:flex-row gap-1 sm:gap-4 py-2 border-b last:border-b-0">
      <div className="w-full sm:w-40 text-sm text-gray-600 font-medium">{label}</div>
            ...
            <ul className="list-disc ml-6 mt-2 space-y-1">
              {attendees.map((a) => (
                ...
                className="break-words break-all">
                  {a.displayName || a.name || "Anonymous"} <br/>
                  ({a.email})
                </li>
              ))}
            </ul>
